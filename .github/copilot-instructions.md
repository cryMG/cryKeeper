...
- `app/config.py` loads optional shared `[crykeeper]` defaults plus optional `[[website]]` TOML tables from `CRYKEEPER_CONFIG_FILE`. Non-empty `CRYKEEPER_*` environment variables override only the shared defaults; matching `[[website]]` entries then override most request-scoped settings per host. Keep `anonymize_client_ip_logs` as a shared/global setting that is not overridden per website. The Docker image default path is `/app/config.toml`.
...
- The Docker image's default Gunicorn access log format should emit the same anonymized client-IP value that cryKeeper uses in its application logs rather than a raw client IP, preferably via the custom logger in `gunicorn.conf.py`. Keep privacy-sensitive logging behavior aligned with `anonymize_client_ip_logs` expectations when you change runtime or demo logging.
...
- `app/routes.py` also supports `enforcement_mode` on the effective request settings. In `log_only`, `GET /check` must still evaluate the normal bypass and cookie logic and log would-challenge decisions, but it should return pass-through instead of `401` so live traffic is not locked out during rollout validation. In `challenge_passthrough`, keep the normal challenge flow, but allow failed `POST /verify` attempts to return the same minimal completion page with a distinct signed passthrough cookie rather than the real human-verification cookie. Keep `POST /verify` issuing the real verification cookie only after real successful verification, and keep URL fragments browser-only by restoring them client-side from the challenge page instead of sending them to the backend.
- `app/observability.py` exposes internal Prometheus metrics and a server-rendered dashboard under the fixed `/_crykeeper` prefix rather than below `path_prefix`. That observability surface includes explicit unsolved challenge attempts based on non-success challenge or verify outcomes plus heuristic runtime warnings for common TLS, proxy, cookie, and auth_request header misconfiguration such as missing `Host`, `User-Agent`, `X-Forwarded-For`, `X-Forwarded-Proto`, or `X-Original-*` forwarding, but it does not try to infer silent browser abandonment or prove an end-to-end rollout. Keep those endpoints suitable for private reverse-proxy exposure and do not accidentally reclassify them as public challenge routes.
...
- `app/routes.py` also emits structured `client_ip` log fields for bypass, log-only, and rate-limit decisions. Keep `_client_ip_value()` as the full sanitized client IP for security logic, but default `anonymize_client_ip_logs` to enabled and log only anonymized `/24` IPv4 or `/48` IPv6 prefixes unless a deployment explicitly disables that privacy guard.
- `app/cookies.py` issues the stateless verification cookie and may bind it to client properties via `CRYKEEPER_HUMAN_COOKIE_BINDING`. Default: `user-agent`; `ip-user-agent` additionally depends on the forwarded client IP. Persist only the HMAC digest of that binding in the cookie payload rather than any raw IP or header value. The default `CRYKEEPER_HUMAN_COOKIE_TTL_SECONDS` is 24 hours unless config overrides it. Keep `secret_key` as the current signing key and `previous_secret_keys` or `CRYKEEPER_PREVIOUS_SECRET_KEYS` as verify-only rotation fallbacks so old cookies can age out without server-side state.
...
- Local cryKeeper JS and CSS entry assets are build-minified into content-hashed filenames plus `app/static/asset-manifest.json` for the published Docker image. Keep Flask rendering resilient when that manifest is absent in source checkouts, keep local asset URL generation aligned between `app/routes.py`, `app/observability.py`, and the build step, keep manifest-backed hashed assets on a 14-day public cache header while leaving unhashed source-checkout assets uncached, and do not introduce a Node.js toolchain for that pipeline unless explicitly requested.
- `app/templates/challenge.html` may also render optional `footer_html` from trusted deployment config. It may be a plain string or a locale-keyed TOML table with English fallback. When no footer is configured, use the built-in cryKeeper footer; when `footer_html` is set to `-`, hide the challenge footer entirely. Keep it request-scoped per host, and do not treat it as user input or sanitize away administrator-provided markup.
- `app/templates/dashboard.html` should always render the built-in default cryKeeper footer instead of any configured `footer_html` override.
