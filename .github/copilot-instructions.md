...
- `app/observability.py` exposes internal Prometheus metrics and a server-rendered dashboard under the fixed `/_crykeeper` prefix rather than below `path_prefix`. That observability surface includes explicit unsolved challenge attempts based on non-success challenge or verify outcomes, but it does not try to infer silent browser abandonment. Keep those endpoints suitable for private reverse-proxy exposure and do not accidentally reclassify them as public challenge routes.
