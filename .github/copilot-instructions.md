...
- `app/routes.py` also supports `skip_routes`, `bypass_user_agents`, `bypass_ips`, and `allow_known_search_engines`, which can bypass `/check` before cookie validation. Keep route matching aligned with reverse-proxy forwarding of `X-Original-URI` and `X-Original-Method`, and keep IP bypass semantics aligned with trusted proxy handling because `bypass_ips` matches the sanitized client address seen by Flask.
- `app/cookies.py` issues the stateless verification cookie and may bind it to client properties via `CRYKEEPER_HUMAN_COOKIE_BINDING`. Default: `user-agent`; `ip-user-agent` additionally depends on the forwarded client IP.
